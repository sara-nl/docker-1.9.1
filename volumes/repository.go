...
func (r *Repository) newVolume(path string, writable bool, driver string) (*Volume, error) {
	...
	if driver == "ceph" {
		isBindMount = false
		driverVolume = path
		path = "/dev/rbd/rbd/" + driverVolume //TODO: This might not actually be the correct path, if the volume is mapped multiple times. How to get the device number (e.g. /dev/rbd0)?
	} else if driver == "nfs" {
		isBindMount = false
	} else {
		isBindMount = true
